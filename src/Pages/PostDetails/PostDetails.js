...
const PostDetails = () => {
  ...
  const { data: comment = [], refetch } = useQuery({
    queryKey: ["comments"],
    queryFn: () =>
      fetch(`https://social-media-server-pi-opal.vercel.app/comments`).then(
        (res) => res.json()
      ),
  });
  console.log(comment);
  ...
  const handleComment = (event) => {
    ...
    fetch("https://social-media-server-pi-opal.vercel.app/addcomment", {
      method: "POST",
      headers: {
    ...
    <div>
      ...
      <div className="card w-3/4 m-10  mx-auto bg-base-100 shadow-xl">
        ...
        <div className="card-body">
          <p>
            <span className="font-bold">Caption:</span> {text}
          </p>
          <div className="card-actions justify-between">
            <div className="text-3xl cursor-pointer mt-3">
              {likeActive ? (
                <FcLike onClick={likeHandler}></FcLike>
              ) : (
                <FcLikePlaceholder onClick={likeHandler}></FcLikePlaceholder>
              )}
            </div>
